...
class Dependency(db.Model):
    ...
    def validate_json(cls, dependency, for_update=False):
        ...
        if not isinstance(dependency, dict) or (dependency.keys() - optional) != required:
            msg = 'A dependency must be a JSON object with the keys name, type, and version.'
            if for_update:
                msg += ' It may also contain the optional key replaces.'
    ...
    def validate_replacement_json(dependency_replacement):
        ...
        if (
            ...
            raise ValidationError(
                'A dependency replacement must be a JSON object with the keys name, type, and '
                'version. It may also contain the optional key new_name.'
            )
...
class Request(db.Model):
    ...
    def add_dependency(self, dependency, replaced_dependency=None):
        """
        Associate a dependency with this request.
