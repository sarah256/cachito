...
@pytest.mark.parametrize('dependency_replacements, error_msg', (
    (
        ['mypackage'],
        'A dependency replacement must be a JSON object with the keys name, type, and version. It '
        'may also contain the optional key new_name.'
    ),
...
@pytest.mark.parametrize('request_id, payload, status_code, message', (
    ...
    (
        ...
        400,
        (
            'A dependency must be a JSON object with the keys name, type, and version. It may also '
            'contain the optional key replaces.'
        ),
    ),
    (
        1,
